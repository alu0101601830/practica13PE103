...
export class FunkoOperations {
    ...
    updateFunko(updatedFunko, username) {
        MongoClient.connect(dbURL).then((client) => {
            ...
            return db.collection('funko').updateOne({ id: updatedFunko.id }, {
                $set: {
    ...
    listFunkos(username) {
        MongoClient.connect(dbURL).then((client) => {
            const db = client.db(dbName);
            const funko = db.collection('funko').find({});
            return funko;
    ...
    loadFunkos() {
        MongoClient.connect(dbURL).then((client) => {
            const db = client.db(dbName);
            const all = db.collection('funko').find().toArray();
    ...
    saveFunko(funko) {
        MongoClient.connect(dbURL).then((client) => {
            const db = client.db(dbName);
            return db.collection('funko').insertOne({
                id: funko.id,
                nombre: funko.nombre,
    ...
    deleteFunkoFile(funkoId) {
        MongoClient.connect(dbURL).then((client) => {
            const db = client.db(dbName);
            return db.collection('funko').deleteOne({
                id: funkoId
            });
        }).then((result) => {
